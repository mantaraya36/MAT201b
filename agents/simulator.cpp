...
struct MyApp : App {
  ...
  void onAnimate(double dt) {
    ...
    //update state
    state.boid_number = c().size();
    for (unsigned i = 0; i < state.boid_number; ++i){
        state.boid_pos[i] = c[i].pose.pos();
        state.boid_quat[i] = c[i].pose.quat();
        state.boid_color[i] = c[i].c;
    }

    state.target_number = tg.targets.size();
    for (unsigned i = 0; i < state.target_number; ++i){
        state.t_pos[i] = tg.targets[i].position;
        state.t_color[i] = tg.targets[i].c;
    }

    state.average_velocity = c.average_velocity();
    state.solitude_level = c.solitude_level();
  }

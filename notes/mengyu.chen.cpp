...
public:
	...
	void onAnimate(double dt){
		...
		for(auto& b : boids){
			...
			//hung target
			float seekUrge = 0.02;
			Vec2f target = Vec2f(px, py); 
			Vec2f desired = (target - b.pos);
			desired *= desired.magSqr();
			b.vel += desired * seekUrge;
			//limit velocity below can cause weird behaviors, not sure why
			//b.vel = b.vel > 0.3 ? 0.3 : b.vel;
		}
	...
	void onKeyDown(const Keyboard& k){
		...
		case 'j': px += -0.05; break;
		case 'l': px += 0.05; break;
		case 'i': py += 0.05; break;
		case 'k': py += -0.05; break;
		}
	}
